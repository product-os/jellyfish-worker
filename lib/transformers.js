	...
	if (
		...
		return Bluebird.map(transformers, async (transformer) => {
			...
			if (match) {
				...
				// transformer
				const [transformerOwner] = await query(transformerOwnerQuery(transformer), { limit: 1 })

				let transformerActor = transformerOwner

				// Or by the actor that owns its contract-repository
				if (!transformerActor) {
					const [repoOwner] = await query(contractRepoOwnerQueryFromTransformer(transformer), { limit: 1 })
