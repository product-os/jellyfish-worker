...
const runExecutor = async (fn, instance, context, session, typeCard, card, options, patch) => {
	return fn(context, instance.jellyfish, session, typeCard, {
		...
		enqueueAction: async (executeSession, actionRequest) => {
			return instance.producer.enqueue(
				instance.getId(), executeSession, actionRequest)
		},
		waitActionResults: async (actionRequest) => {
			return instance.producer.waitResults(context, actionRequest)
		}
	}, card, patch)
}
...
const parseTrigger = (context, trigger) => {
	...
	assert.INTERNAL(context,
		trigger.actor && _.isString(trigger.actor),
		errors.WorkerInvalidTrigger, `Invalid actor: ${trigger.actor}`)
...
exports.Worker = class Worker {
	...
	async tick (context, session, options) {
		...
		await Bluebird.map(currentTriggers, async (trigger) => {
			...
			const triggerSession = await utils.getActorKey(
				context, this.jellyfish, this.jellyfish.sessions.admin, trigger.actor)
