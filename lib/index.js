...
const runExecutor = async (fn, instance, context, session, typeCard, card, options, patch) => {
	return fn(context, instance.jellyfish, session, typeCard, {
		...
		waitResults: async (requestContext, actionRequest) => {
...
exports.Worker = class Worker {
	...
	/**
	 * @summary Updates the list of transformers, such that only the latest version of each major version exists
	 * @function
	 * @private
	 *
	 * @example
	...
	updateLatestTransformers () {
		...
		this.transformers.forEach((tf) => {
			...
			if (!transformers[slugMajV] || semver.gt(tf.version, transformers[slugMajV].version)) {
	 ...
	 */
	upsertTransformer (context, transformer) {
	 ...
	 */
	upsertSubscription (context, subscription) {
