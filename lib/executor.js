...
const commit = async (context, jellyfish, session, typeCard, current, options, fn) => {
	...
	if (options.transformers) {
		Bluebird.map(options.transformers, async (transformer) => {
			// TODO: See if there is there a way to allow transformer before trigger to match
			// $$links
			const beforeMatch = transformer.data.trigger.before && skhema.isValid(transformer.data.trigger.before, current)

			if (options.eventType === 'update' && !beforeMatch) {
				return
			}

			// TODO: Re-use triggers code that has a lot of optimisation around links
			const afterMatch = transformer.data.trigger.after && skhema.isValid(transformer.data.trigger.after, insertedCard)

			if (
				(options.eventType === 'create' && afterMatch) ||
				(options.eventType === 'update' && afterMatch)
			) {
				// The transformer should be run on behalf of the actor that owns the
				// transformer
				const [ transformerActor ] = await jellyfish.query(context, options.context.privilegedSession, {
					$$links: {
						owns: {
							type: 'object',
							properties: {
				...
				if (transformerActor) {
					const actionRequest = {
						...
						arguments: {
							reason: null,
							properties: {
								name: `Transform ${insertedCard.id} using transformer ${transformer.id}`,
								data: {
									input: insertedCard,
									transformer,
									actor: transformerActor.id,
					...
					await exports.insertCard(context, jellyfish, options.context.privilegedSession,
						await jellyfish.getCardBySlug(context, options.context.privilegedSession, 'link@1.0.0'),
						{
							...options,
							attachEvents: true,
							timestamp: Date.now(),
