...
const commit = async (context, jellyfish, session, typeCard, current, options, fn) => {
	...
	if (options.transformers) {
		transformers.evaluate({
			...
			query: (querySchema, queryOpts) => {
				return jellyfish.query(context, options.context.privilegedSession, querySchema, queryOpts)
			},
			executeAndAwaitAction: async (actionRequest) => {
				actionRequest.context = context
				const req = await options.executeAction(options.context.privilegedSession, actionRequest)
	...
	if (options.subscriptions) {
		Bluebird.map(options.subscriptions, async (subscription) => {
			try {
				const view = _.find(subscription.links['is attached to'], {
					type: 'view@1.0.0'
				})

				const createCard = _.find(subscription.links['has attached element'], {
					type: 'create@1.0.0'
				})

				assert.INTERNAL(context, createCard && createCard.data && createCard.data.actor,
					errors.WorkerNoElement,
				...
				const notification = await exports.insertCard(context, jellyfish, creatorSession.id,
					await jellyfish.getCardBySlug(context, session, 'notification@1.0.0'),
					{
						...options,
						attachEvents: true,
						timestamp: Date.now(),
				...
				await exports.insertCard(context, jellyfish, creatorSession.id,
					await jellyfish.getCardBySlug(context, session, 'link@1.0.0'),
					{
						...options,
						attachEvents: true,
						timestamp: Date.now(),
					...
					},
					{
						version: '1.0.0',
						type: 'link@1.0.0',
						slug: `link-${filteredCard.id}-has-attached-${notification.id}-${await uuid.random()}`,
						tags: [],
						links: {},
						requires: [],
						capabilities: [],
			...
			return Promise.all(triggerCards.map((triggerCard) => {
				...
				return trigger.schedule === 'enqueue'
					...
					: options.library[request.action.split('@')[0]].handler(
						triggerSession, options.context, triggerCard, actionRequest
					)
			}))
		}
...
exports.insertCard = async (context, jellyfish, session, typeCard, options, object) => {
	...
	return commit(context, jellyfish, session, typeCard, card, options, async () => {
		const objectToEvaluate = await getEvaluatableObject(context, session, jellyfish, object, typeCard)
		const result = jellyscript.evaluateObject(typeCard.data.schema, objectToEvaluate)
...
exports.patchCard = async (context, jellyfish, session, typeCard, options, object, patch) => {
	...
	return commit(context, jellyfish, session, typeCard, object, options, async () => {
		...
		const objectToEvaluate = await getEvaluatableObject(context, session, jellyfish, card, typeCard)
		const newPatch = jellyscript.evaluatePatch(typeCard.data.schema, objectToEvaluate, patch)
 ...
 * @param {Object} typeCard - type card
 *
 * @returns {Object} - the object with any links referenced in the evaluated fields
 * of it's type card's schema.
 */
const getEvaluatableObject = async (context, session, jellyfish, object, typeCard) => {
	const linkVerbs = utils.getReferencedLinkVerbs(typeCard)
	if (!linkVerbs.length) {
		return object
	}
	const query = utils.getQueryWithOptionalLinks(object, linkVerbs)
	const [ queriedCard ] = await jellyfish.query(context, session, query)
	const cardWithLinks = queriedCard || object
