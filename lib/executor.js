...
const commit = async (context, jellyfish, session, typeCard, current, options, fn) => {
	...
	if (options.triggers) {
		const runTrigger = async (trigger) => {
			...
			const triggerSession = await utils.getActorKey(context, jellyfish, jellyfish.sessions.admin, trigger.actor)

			const request = await triggers.getRequest(jellyfish, trigger, insertedCard, {
				currentDate: new Date(),
				mode: current ? 'update' : 'insert',
				context,
				session: triggerSession.id
			})
			...
			return Bluebird.map(identifiers, async (identifier) => {
				const triggerTargetCard = await getInputCard(
					context, jellyfish, triggerSession.id, identifier)

				assert.INTERNAL(context, triggerTargetCard, errors.WorkerNoElement,
					`No such input card for trigger ${trigger.slug}: ${identifier}`)

				const actionRequest = await options.enqueueAction(triggerSession.id, {
					...request,
					card: triggerTargetCard.id,
				...
				const actionResult = await options.waitActionResults(actionRequest)

				if (actionResult.error) {
					throw errio.fromObject(actionResult.data)
				}
	...
	if (insertedCard.type === CARD_TYPE_TYPE) {
		...
		const triggerTypeCard = await jellyfish.getCardBySlug(
			context, jellyfish.sessions.admin, 'triggered-action@1.0.0')
		...
		await Bluebird.map(jellyscript.getTypeTriggers(insertedCard), async (trigger) => {
			...
			const insertedTrigger = await options.context.replaceCard(session, triggerTypeCard, {
				attachEvents: true,
